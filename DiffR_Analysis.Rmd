...
```{r scenario-1-dataset, include=FALSE}
...
## ----------create artificial group of odd and even time points -------##
...
```{r scenario-1-analysis, include=FALSE}
...
f1_2_1 <- f_venn + 
  geom_text(aes(x=x,y=y,label=paste0(label, " \n", counts)), 
            data=df1_2_1, color="white", fontface="bold.italic", size=5/14*6) + 
  labs(title = "FDR  < 0.05") 
...
f1_2_2 <- f_venn  + 
  geom_text(aes(x=x,y=y,label=paste0(label, " \n", counts)), 
            data=df1_2_2, color="white", fontface="bold.italic", size=5/14*6) + 
...
f1_4 <- f_venn +
  geom_text(aes(x=x,y=y,label=paste0(label, " \n", counts, "\n", TP)), 
            data=df1_4, color="white", fontface="bold.italic", size=5/14*6) + 
  labs(title = "FDR < 0.05")
...
f1_5 <- ggplot(df1_5, aes_(x=~TP/(TP+FN), y=~TP/(FP+TP))) + 
  ...
  annotate("text", x=0.29, y = 0.3, color="grey30", label="increasing \nFDR threshold", hjust=0, size=5/14*6.5) +
...
f1_6 <- f_venn + 
  geom_text(aes(x=x,y=y,label=paste0(label, " \n", counts, "\n", TP)), 
            data=df1_6, color="white", fontface="bold.italic", size=5/14*6) + 
  labs(title = paste0("FDR < ", scales::scientific(df1_5[point_1, "fdr_cutoff"], digits=2)))
```

```{r Figure1, echo=FALSE, fig.height=5.75, fig.width=6.5}
...
```{r scenario-1-supp, include=FALSE}
...
fS1_6 + fS1_7 + fS1_1_1 + fS1_1_2 + fS1_2 + fS1_3 + fS1_4 + fS1_5 +  plot_layout(design = layout, widths = c(1,1,1.05)) + 
  plot_annotation(tag_levels = 'A')
...
```{r scenario-2-compareRhythms, include=FALSE}
...
```{r Figure3, fig.height=3, fig.width=6.5}
...
dfS2_1 <- evalmod(scores = join_scores(-log10(dodr_scenario_2_1$adj_p_val_dodr),
...
dfS2_2 <- evalmod(scores = join_scores(-log10(dodr_scenario_2_1$adj_p_val_dodr),
                                       -log10(dodr_scenario_2_2$adj_p_val_dodr)),
...
dfS2_3 <- bind_rows(df3_1, df3_2, dfS2_1, dfS2_2) %>%
  dplyr::select(c("x", "y", "amp_cutoff", "method", "dset")) %>%
...
ggplot(dfS2_3, aes(x=x, y = y, color=method, linetype=factor(amp_cutoff))) + geom_line(size=0.5) + 
  geom_point(shape=21, fill="white", data=filter(op_points, dset=="high"), stroke=0.75, show.legend = FALSE) +
  facet_grid(cols = vars(factor(dset, levels = c("low", "high"))), 
             labeller = as_labeller(c(low = sprintf("Target: all %d true DiffR transcripts", length(rhythmic_in_A)), 
                                      high = sprintf("Target: %s large ampl. true DiffR transcripts", length(rhythmic_in_A_large_amp))))) + 
...
```{r HFD-timecourses, fig.width=6.5, fig.height=3.5}
...
dfS3_1 <- data.frame(source = rep(c("Eckel-Mahan", "Quagliarini"), each=4),
                     category = rep(c("DR", "ABR"), 4),
                     method = rep(c("hypothesis testing", "model selection"), 2, each=2),
...
fS3_1 <- ggplot(subset(dfS3_1, category=="DR")) +
  geom_col(aes(x=method, y=total, color=source), fill="white", width = 0.55) +
  geom_col(aes(x=method, y=counts, fill=source), width = 0.55) +
  facet_wrap(~source, nrow=1) +
  geom_text(aes(x=method, y=counts, label=scales::percent(fraction, accuracy = 1),), 
            vjust=-0.5, data = subset(dfS3_1, category=="DR"), size=5/14*7)+
  scale_fill_tableau(palette="Superfishel Stone") + guides(color="none") +
  scale_color_tableau(palette="Superfishel Stone") +
  theme(strip.text = element_blank(), legend.key.height = unit(0.25, "lines"),
        legend.spacing.x = unit(0.5, "lines")) +
...
dfS3_2 <- tableGrob(table(compare_results_nutr_1[,-1]), theme = tt) %>%
...
dfS3_3 <- tableGrob(table(compare_results_nutr_2[,-1]), theme = tt) %>%
  gtable_add_cols(grobWidth(row_title) + unit(1, "line"),pos = 0) %>%
  gtable_add_grob(row_title, t=1, l=1, b=nrow(.), r=1) %>%
  gtable_add_rows(grobHeight(col_title) + unit(1, "line"),pos = 0) %>%
  gtable_add_grob(col_title, t=1, l=1, r=ncol(.), b=1) %>%
...
dfS3_4 <- tableGrob(table(compare_results_nutr_3[,-1]), theme = tt) %>%
  gtable_add_cols(grobWidth(row_title) + unit(1, "line"),pos = 0) %>%
  gtable_add_grob(row_title, t=1, l=1, b=nrow(.), r=1) %>%
  gtable_add_rows(grobHeight(col_title) + unit(1, "line"),pos = 0) %>%
  gtable_add_grob(col_title, t=1, l=1, r=ncol(.), b=1) %>%
...
dfS3_5 <- data.frame(x=c(-0.5, 0.5, 1.5), y = 0,
                     label=c("rhythmic \nin WT-alone", "rhythmic\n in BOTH", "rhythmic \nin HF-alone"))

dfS3_5$counts <- c(1394,778,654)

fS3_5 <- f_venn + 
  geom_text(aes(x=x,y=y,label=paste0(label, " \n", counts)), 
            data=dfS3_5, color="white", fontface="bold.italic", size=5/14*7) +
  labs(title = "P < 0.01")
...
dfS3_6 <- tableGrob(table(compare_results_nutr_4[,-1]), theme = tt) %>%
  gtable_add_cols(grobWidth(row_title) + unit(1, "line"),pos = 0) %>%
  gtable_add_grob(row_title, t=1, l=1, b=nrow(.), r=1) %>%
  gtable_add_rows(grobHeight(col_title) + unit(1, "line"),pos = 0) %>%
  gtable_add_grob(col_title, t=1, l=1, r=ncol(.), b=1) %>%
...
fS3_1 + wrap_elements(panel=dfS3_2) + wrap_elements(panel=dfS3_3) + wrap_elements(full=dfS3_4) + wrap_elements(panel=fS3_5, clip=FALSE) + wrap_elements(full=dfS3_6) + plot_layout(design = layout, heights =  c(0.75,1), widths = c(1,0.2,1,1)) + plot_annotation(tag_levels = 'A')
...
dfS4_1 <- data.frame(category = rep(c("DR", "ABR"), 2),
...
dfS4_2 <- tableGrob(table(compare_results_keto[,-1]), theme = tt) %>%
  gtable_add_cols(grobWidth(row_title) + unit(1, "line"),pos = 0) %>%
  gtable_add_grob(row_title, t=1, l=1, b=nrow(.), r=1) %>%
  gtable_add_rows(grobHeight(col_title) + unit(1, "line"),pos = 0) %>%
  gtable_add_grob(col_title, t=1, l=1, r=ncol(.), b=1) %>%
...
dfS4_3 <- data.frame(x=c(-0.5, 0.5, 1.5), y = 0,
                     label=c("rhythmic \nonly in CC", "rhythmic\n in Both", "rhythmic \nonly in KD"))

dfS4_3$counts <- c(719, 801, 2339)

fS4_3 <- f_venn + 
  geom_text(aes(x=x,y=y,label=paste0(label, " \n", counts)), 
            data=dfS4_3, color="white", fontface="bold.italic", size=5/14*7) +
  labs(title = "P < 0.01")
...
dfS4_4 <- tableGrob(table(compare_results_keto_2[,-1]), theme = tt) %>%
  gtable_add_cols(grobWidth(row_title) + unit(1, "line"),pos = 0) %>%
  gtable_add_grob(row_title, t=1, l=1, b=nrow(.), r=1) %>%
  gtable_add_rows(grobHeight(col_title) + unit(1, "line"),pos = 0) %>%
  gtable_add_grob(col_title, t=1, l=1, r=ncol(.), b=1) %>%
...
wrap_elements(full=dfS4_2) + fS4_3 + wrap_elements(full=dfS4_4) + plot_layout(nrow = 1, ncol=3, widths = c(1,0.9,1)) + plot_annotation(tag_levels = 'A')
...
```{r p66ShcKO-DiffR-analysis, include=FALSE}
...
```{r FigureS5, fig.height=2, fig.width=6.5}
...
dfS5_2 <- tableGrob(table(compare_results_pei[,-1]), theme = tt) %>%
  gtable_add_cols(grobWidth(row_title) + unit(1, "line"),pos = 0) %>%
  gtable_add_grob(row_title, t=1, l=1, b=nrow(.), r=1) %>%
  gtable_add_rows(grobHeight(col_title) + unit(1, "line"),pos = 0) %>%
  gtable_add_grob(col_title, t=1, l=1, r=ncol(.), b=1) %>%
...
dfS5_4 <- tableGrob(table(compare_results_pei_2[,-1]), theme = tt) %>%
  gtable_add_cols(grobWidth(row_title) + unit(1, "line"),pos = 0) %>%
  gtable_add_grob(row_title, t=1, l=1, b=nrow(.), r=1) %>%
  gtable_add_rows(grobHeight(col_title) + unit(1, "line"),pos = 0) %>%
  gtable_add_grob(col_title, t=1, l=1, r=ncol(.), b=1) %>%
...
```{r WTvsWT, eval=TRUE, fig.width=3}
