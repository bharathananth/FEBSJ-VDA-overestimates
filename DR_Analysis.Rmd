...
```{r Figure1, fig.width = 7, fig.height=2.5, eval=TRUE}
...
f1_2 <- ggplot(df1_2) +
  ...
  xlab("false discovery cutoff for rhythms") + ylab("DiffR genes") +
  scale_x_continuous(trans = "log10", expand = c(0,0), breaks = c(0.0001, 0.001, 0.01, 0.1),
                     labels = scales::trans_format("log10", scales::math_format(10^.x))) +
  coord_cartesian(xlim = c(0.00001,0.5)) + scale_y_continuous(expand = c(0.01,0))


f1_1 + f1_2 + plot_layout(nrow=1, widths = c(1,2)) + plot_annotation(tag_levels = 'A')

```

```{r artificial-DR-rocs}
ggplot(df1_2, aes_(x=~TP/(TP+FN), y=~TP/(FP+TP), linetype=~factor(amp_cutoff))) + 
  geom_hline(aes_(yintercept=~N/(N + nrow(pvm_ex_2))), color="grey90") +
  geom_path(color="black") +
  xlab("Fraction of DiffR transcripts found (Recall)") + ylab("Fraction of true DiffR transcripts (Precision)") + coord_cartesian(xlim=c(0,1.0), ylim=c(0,1.0))
...
```{r analyze-odd-even-supp, fig.width=4, eval=TRUE}
...
```{r artificial-DR-data-supp, eval=TRUE}
N <- 0.1*nrow(pvm_ex_S1)
...
```{r FigureS1, fig.width=7, fig.height=2.5, eval=TRUE}
