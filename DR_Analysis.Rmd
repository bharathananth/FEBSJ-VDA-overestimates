...
```{r Figure6, fig.width=5, fig.height=3}
...
df6 <- bind_rows(df6_1, df6_2, df6_3, df6_4, df6_5, df6_6) %>%
...
op_points <- lapply(list(limma_ex_2_2, dodr_ex_2_2, mod_sel_ex_2_2),
       function(d) {
         if ("weights" %in% colnames(d)) {
          ...
          tp <- c(length(intersect(rhythmic_in_A, est_dr_genes$symbol)),
                  length(intersect(rhythmic_in_A_large_amp, est_dr_genes$symbol)))
          fn <- c(length(setdiff(rhythmic_in_A, est_dr_genes$symbol)),
                  length(setdiff(rhythmic_in_A_large_amp, est_dr_genes$symbol)))
          fp <- c(length(setdiff(est_dr_genes$symbol, rhythmic_in_A)),
                  length(setdiff(est_dr_genes$symbol, rhythmic_in_A_large_amp)))
          x <- tp/(tp + fn)
          y <- tp/(tp + fp)
...
ggplot(df6, aes(x=x, y = y, color=method, linetype=factor(amp_cutoff))) + geom_line(size=0.5) + 
