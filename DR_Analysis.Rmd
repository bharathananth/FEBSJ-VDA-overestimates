...
exp_des_nut_1 <- colData(se) %>% as.data.frame %>% tibble() %>%
                 select(!!c("harvest.timepoint.ch1", "diet.ch1")) %>%
...
compare_res <- join(res_dodr, res_limma, by="symbol")
...
exp_des_nut_2 <- colData(se) %>% as.data.frame %>% tibble %>%
                 select(!!c("time", "group")) %>%
...
df <- data.frame(source = rep(c("Eckel-Mahan et al.", "Quagliarini et al."), each=2),
                 category = rep(c("DR", "ABR"), 2),
                 counts = c(table(res_limma$category == "ABR"), table(res_voom$category == "ABR"))) %>%
      group_by(source) %>%
      mutate(fraction = counts/sum(counts)) %>%
      mutate(ymax = cumsum(fraction), ymin = c(0, head(ymax, n=-1)),
             labelPosition = (ymax + ymin) / 2)

ggplot(df, aes(ymin=ymin, ymax=ymax, xmin=3, xmax=4, fill=category)) + 
  geom_rect(color="white") + coord_polar(theta = "y") + facet_wrap(~source, nrow = 1) +
  geom_text( x=3.5, aes(y=labelPosition, label=counts), size=3.5, color="white", fontface="bold") +
  xlim(c(2,4)) + theme_void() + theme(legend.position = "none", strip.background = element_blank()) + 
  scale_fill_brewer(palette="Accent")
```

```{r nut_phase_shift}
nut_compare <- inner_join(res_limma, res_voom, by="symbol") %>%
...
with(nut_compare, table(diff_rhythmic.x, diff_rhythmic.y))

df <- filter(nut_compare, DR) %>%
      select(amp_change_1, phase_change_1, amp_change_2, phase_change_2, symbol) %>%
      gather(var, value, -symbol) %>%
      separate(var, c("var", "junk", "source")) %>% select(-junk) %>%
...
ggplot(data=df) +
  ...
  scale_y_continuous(breaks = seq(-2*pi/3, pi, pi/3), limits=c(-pi,pi), 
                     labels = (12/pi*seq(-2*pi/3, pi, pi/3)) %% 24) +
...
```{r go_and_kegg}
go <- enrichGO(nut_compare$symbol[nut_compare$DR], universe = nut_compare$symbol, OrgDb = org.Mm.eg.db, keyType = "ENSEMBL", minGSSize = 1, pvalueCutoff = 0.2, pAdjustMethod = "none", ont = "BP", qvalueCutoff = 1.0)

dr_entrez <- subset(rowData(se), ensembl_gene_id %in% nut_compare$symbol)
kegg <- enrichKEGG(dr_entrez$entrezgene_id[nut_compare$DR], universe = as.character(dr_entrez$entrezgene_id), organism = "mmu", keyType = "ncbi-geneid", pvalueCutoff = 1.0, qvalueCutoff = 1.0, minGSSize = 5)

go_1 <- enrichGO(res_limma$symbol[res_limma$diff_rhythmic], universe = res_limma$symbol, OrgDb = org.Mm.eg.db, keyType = "ENSEMBL", minGSSize = 1, pvalueCutoff = 0.05, pAdjustMethod = "none", ont = "BP", qvalueCutoff = 1.0)

go_2 <- enrichGO(res_voom$symbol[res_voom$diff_rhythmic], universe = res_voom$symbol, OrgDb = org.Mm.eg.db, keyType = "ENSEMBL", minGSSize = 1, pvalueCutoff = 0.05, pAdjustMethod = "none", ont = "BP", qvalueCutoff = 1.0)

dr_entrez <- subset(rowData(se), ensembl_gene_id %in% nut_compare$symbol)
kegg_1 <- enrichKEGG(dr_entrez$entrezgene_id[nut_compare$DR], universe = as.character(dr_entrez$entrezgene_id), organism = "mmu", keyType = "ncbi-geneid", pvalueCutoff = 1.0, qvalueCutoff = 1.0, minGSSize = 5)
...
tft <- enricher(dr_entrez$mgi_symbol[nut_compare$DR], universe = dr_entrez$mgi_symbol, TERM2GENE = m_t2g, qvalueCutoff = 0.25, pAdjustMethod = "none", pvalueCutoff = 0.05)
```

```{r reactome}
m_df <- msigdbr(species = "Mus musculus", category = "C2", subcategory = "CP:REACTOME")
m_t2g <- m_df %>% dplyr::select(gs_name, gene_symbol) %>% as.data.frame()
reactome <- enricher(dr_entrez$mgi_symbol[nut_compare$DR], universe = dr_entrez$mgi_symbol, TERM2GENE = m_t2g, qvalueCutoff = 1.0, pAdjustMethod = "none", pvalueCutoff = 0.05)
```

```{r}
m_list <- m_df %>% split(x = .$gene_symbol, f=.$gs_name)
nut_compare %<>% mutate(pv = pmin(adj_p_val_DR.x, adj_p_val_DR.y)) %>% dplyr::arrange(pv)
test <- fgsea(pathways=m_list, stats = )
```
