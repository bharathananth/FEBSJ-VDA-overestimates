...
keep <- apply(y_nutr_1, 1, function(x) median(x[exp_des_nutr_1$group == group_ids[1]])>5 | 
                                median(x[exp_des_nutr_1$group == group_ids[2]])>5 )

y_nutr_1 <- y_nutr_1[keep, ]

results_dodr <- compareRhythms(y_nutr_1, exp_des_nutr_1, method = "dodr", just_classify = FALSE, 
                           compare_fdr = 0.05)
...
compare_res <- inner_join(results_dodr, results_emahan, by="symbol")
...
```{r nutr_challenge_2, fig.align="center"}
...
keep <- edgeR::filterByExpr(y_nutr_2, group=exp_des_nutr_2$group)  #rownames(y_nutr_2) %in% rownames(y_nutr_1)
...
with(nutr_compare, table(diff_rhythmic.x, diff_rhythmic.y))

df <- filter(nutr_compare, DR) %>%
