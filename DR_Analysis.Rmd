...
df_1_1 <- expand.grid(fdr_cutoff = 10^seq(-3.5, 0, by = 0.2),
                  amp_cutoff = amp_choices)
df_1_1$no_of_DR <- vapply(seq(nrow(df_1_1)),
                   function(i) {
                     return(sum(xor(pvm_ex_1$adj_p_val_A < df_1_1[i, "fdr_cutoff"] &
                                    pvm_ex_1$A_amp > df_1_1[i, "amp_cutoff"],
                                    pvm_ex_1$adj_p_val_B < df_1_2[i, "fdr_cutoff"] &
                                    pvm_ex_1$B_amp > df_1_1[i, "amp_cutoff"])))
                   }, FUN.VALUE = numeric(1))
df_1_1$percent <- df_1_1$no_of_DR/nrow(liver_hughes)
...
```{r artificial-DR-data-analysis, fig.width = 7, fig.height=2.5, eval=FALSE}
pvm_ex_2 <- rhythm_analysis(new_expr, exp_design)

df_1_2 <- expand.grid(fdr_cutoff = 10^seq(-6, log10(0.25), by=0.05),
                  amp_cutoff = amp_choices)
true_dr_genes <- rownames(pvm_ex_2) %in% rhythmic_in_A

roc <- vapply(seq(nrow(df_1_2)),
              function(i) {
                est_dr_genes <- xor(pvm_ex_2$adj_p_val_A < df_1_2[i, "fdr_cutoff"] &
                                      pvm_ex_2$A_amp > df_1_2[i, "amp_cutoff"],
                                    pvm_ex_2$adj_p_val_B < df_1_2[i, "fdr_cutoff"] &
                                      pvm_ex_2$B_amp > df_1_2[i, "amp_cutoff"])
                tp <- sum(true_dr_genes & est_dr_genes)
                fp <- sum(!true_dr_genes & est_dr_genes)
                return(c(TP = tp, FP = fp))
              }, FUN.VALUE = numeric(2))
df_1_2 <- cbind(df_1_2, t(roc))
...
f4_1 <- ggplot(df4_1) +
  ...
  theme(legend.title = element_blank(), legend.key.size = unit(0.75,"line"),
        legend.justification = "left") + ylab("number of DR genes") + xlab("")


df4_2 <- results_keto %>%
         filter(diff_rhythmic) %>%
                 mutate(amp_change = (Ketogenic_amp - control_amp),
                      phase_change = exp(1i*Ketogenic_phase - 1i*control_phase))

f4_2 <- ggplot(data=df4_2) +
  geom_vline(xintercept = c(-1,1), linetype="dashed", color="grey80") +
  geom_vline(xintercept = 0, color = "grey80") +
  geom_hline(yintercept = seq(-2*pi/3, pi, pi/3), color="grey80") +
  geom_point(aes(x=amp_change, y = Arg(phase_change), color=category), size=0.4) +
  theme(aspect.ratio = 1) + xlab("") + ylab("") + scale_color_tableau("Classic 10") +
  geom_text(data=data.frame(x=seq(-1,1), y=pi/2), aes(x=x, y=y, label=x), nudge_x = -0.2, size=9*0.35) + 
  scale_x_continuous(breaks=seq(-2,1), limits = c(-2,1), expand = c(0.1,0)) + 
  scale_y_continuous(breaks = seq(-2*pi/3, pi, pi/3), limits=c(-pi,pi), 
                     labels = ((12/pi*seq(-2*pi/3, pi, pi/3) + 12) %% 24) - 12, ) +
...
```{r enrichment, fig.height=3, fig.width=8}
...
f4_1 + f4_2 + gtable_rbind(f4_3_1, f4_3_2) + plot_layout(nrow = 1) + plot_annotation(tag_levels = "a")
...
```{r enrichment_iec, fig.height=3, fig.width=7.5}
...
dfS4_1 <- data.frame(table(results_keto_iec$category)) %>%
         rename(category = Var1, freq = Freq)

fS4_1 <- ggplot(df4_1) +
  geom_col(aes(x=category, y=freq), position = "dodge", width = 0.5) +
  annotate("text", x = 3, y = c(950, 800), label= c("A = control", "B = ketogenic"), hjust = 0, size = 0.35*9) +
  ylab("number of DR genes") + xlab("")
...
fS4_2 <- ggplot(data=dfS4_2) +
  ...
  geom_vline(xintercept = 0, color = "grey80") +
  geom_hline(yintercept = seq(-2*pi/3, pi, pi/3), color="grey80") +
  geom_point(aes(x=amp_change, y = Arg(phase_change), color=category, shape = category), size=0.75) +
...
dfS4_3_1 <- head(kegg_iec, n=3) %>%
           mutate(Description = str_wrap(Description, width=20),
                  qvalue = scales::comma(qvalue, digits = 3)) %>%
           dplyr::select(Description, qvalue)
...
fS4_1 + fS4_2 + gtable_rbind(fS4_3_1, fS4_3_2) + plot_layout(nrow = 1, widths = c(0.8,1,1)) + plot_annotation(tag_levels = "a")
...
```{r pei_enrichment, fig.height=3, fig.width=7.5}
...
f5_3_2 <- tableGrob(df5_3_2, theme = ttheme_default(base_size = 8), cols = c("Hallmark genes(MSigDB)", "qvalue"), rows = NULL)


f5_1 + f5_2 + gtable_rbind(f5_3_1, f5_3_2) + plot_layout(nrow = 1, widths = c(0.8,1,1)) + plot_annotation(tag_levels = "a")
```
