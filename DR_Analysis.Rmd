...
exp_design <- colData(se)[, c("harvest.timepoint.ch1", "diet.ch1")]
exp_design <- as.data.frame(exp_design)
exp_design$time <- as.numeric(sub("ZT", "", exp_design$harvest.timepoint.ch1))
exp_design$group <- factor(make.names(exp_design$diet.ch1))
y_nut_1 <- assay(se, "expr")

group_ids <- levels(exp_design$group)
keep <- apply(y_nut_1, 1, function(x) median(x[exp_design$group == group_ids[1]])>5 | 
                                median(x[exp_design$group == group_ids[2]])>5 )

y_nut_1 <- y_nut_1[keep, ]

res_dodr <- compareRhythms(y_nut_1, exp_design, method = "dodr", just_classify = FALSE, compare_fdr = 0.05)
res_limma <- compareRhythms(y_nut_1, exp_design, method = "limma", just_classify = FALSE, compare_fdr = 0.05)
...
grid.table(df, rows=c("not DR", "DR"), cols=c("not DR", "DR"), theme=tt)

```

```{r nut_challenge_1, fig.align="center"}
se <- readRDS("data/PRJNA428303.RDS")
exp_design <- colData(se)[, c("time", "group")]
exp_design <- as.data.frame(exp_design)
exp_design$time <- as.numeric(sub("ZT", "", exp_design$time))
exp_design$group <- factor(exp_design$group)
y_nut_2 <- assay(se, "countsFromAbundance")

group_ids <- levels(exp_design$group)
design <- model.matrix(~group + time, data=exp_design)
keep <- edgeR::filterByExpr(y_nut_2, design = design, min.total.count = 25)

y_nut_2 <- y_nut_2[keep, ]

res_voom <- compareRhythms(y_nut_2, exp_design, method = "voom", just_classify = TRUE, compare_fdr = 0.05, outliers = TRUE, amp_cutoff = 0.01)
