...
levels(exp_des_nut_1$group) <- list(HFD="high.fat.diet", control="normal.chow")
...
```
```{r nut_phase_shift}
nut_compare <- merge(res_limma, res_voom, by="symbol") %>%
...
table(nut_compare$diff_rhythmic.x, nut_compare$diff_rhythmic.y)

df <- subset(nut_compare, DR) %>%
