...
f1_2 <- ggplot(df1_2) +
  ...
  coord_cartesian(xlim = c(0.00001,0.5)) + scale_y_continuous(expand = c(0.01,0)) + theme(panel.spacing.y = unit(1.2, "lines"))
...
df1_2_2 <- expand.grid(fdr_cutoff = 10^seq(-9, log10(0.999), by=0.1),
                  amp_cutoff = amp_choices)
rhythmic_in_A_large_amp <- rhythmic_in_A[pvm_ex_2[rhythmic_in_A, "A_amp"] > 0.5]
...
```{r Figure6, fig.width=6.5, fig.height=4}
all_genes <- data.frame(symbol = rownames(new_expr))
...
limma_ex_2_1 <- compareRhythms(new_expr, exp_design = exp_design, method = "limma",
                ...
                right_join(all_genes, by = "symbol") %>% arrange(symbol)
...
df6_1 <- evalmod(scores = join_scores(-log10(limma_ex_2_1$adj_p_val_DR),
                                      -log10(limma_ex_2_2$adj_p_val_DR)),
        labels = limma_ex_2_1$symbol %in% rhythmic_in_A,
        modnames = as.character(amp_choices)) %>%
        fortify(reduce_points=TRUE) %>% filter(curvetype == "PRC")


f6_1 <- ggplot(df6_1, aes(x=x, y=y, linetype=modname)) + 
  geom_hline(aes_(yintercept=~N/(N + nrow(limma_ex_2_1))), color="grey60", linetype="twodash") +
  geom_path(color=colorblind_pal()(3)[1]) + 
  scale_linetype_manual(name="", labels=parse(text = paste0('A > ', amp_choices)), 
                        values = c("solid", "dashed")) + 
  ylab("Precision = true DiffR hits/ DiffR hits") + coord_cartesian(xlim=c(0,1.0), ylim=c(0,1.0))

df6_2 <- evalmod(scores = join_scores(-log10(limma_ex_2_1$adj_p_val_DR),
                                      -log10(limma_ex_2_2$adj_p_val_DR)),
        labels = limma_ex_2_1$symbol %in% rhythmic_in_A_large_amp,
        modnames = as.character(amp_choices)) %>%
        fortify(reduce_points=TRUE) %>% filter(curvetype == "PRC")

f6_2 <- ggplot(df6_2, aes(x=x, y=y, linetype=modname)) + 
  geom_hline(aes_(yintercept=~N/(N + nrow(limma_ex_2_1))), color="grey60", linetype="twodash") +
  geom_path(color=colorblind_pal()(3)[1]) + 
  scale_linetype_manual(name="", labels=parse(text = paste0('A > ', amp_choices)), 
                        values = c("solid", "dashed")) + 
...
df6_3 <- evalmod(scores = join_scores(-log10(dodr_ex_2_1$adj_p_val_dodr),
                                      -log10(dodr_ex_2_2$adj_p_val_dodr)),
        labels = dodr_ex_2_1$symbol %in% rhythmic_in_A,
        modnames = as.character(amp_choices)) %>%
        fortify(reduce_points=TRUE) %>% filter(curvetype == "PRC")

f6_3 <- ggplot(df6_3, aes(x=x, y=y, linetype=modname)) + 
  geom_hline(aes_(yintercept=~N/(N + nrow(dodr_ex_2_1))), color="grey60", linetype="twodash") +
  geom_path(color=colorblind_pal()(3)[2]) + 
  scale_linetype_manual(name="", labels=parse(text = paste0('A > ', amp_choices)), 
                        values = c("solid", "dashed")) + 
 coord_cartesian(xlim=c(0,1.0), ylim=c(0,1.0)) + theme(legend.position = "none")

df6_4 <- evalmod(scores = join_scores(-log10(dodr_ex_2_1$adj_p_val_dodr),
                                      -log10(dodr_ex_2_2$adj_p_val_dodr)),
        labels = dodr_ex_2_1$symbol %in% rhythmic_in_A_large_amp,
        modnames = as.character(amp_choices)) %>%
        fortify(reduce_points=TRUE) %>% filter(curvetype == "PRC")

f6_4 <- ggplot(df6_4, aes(x=x, y=y, linetype=modname)) + 
  geom_hline(aes_(yintercept=~N/(N + nrow(dodr_ex_2_1))), color="grey60", linetype="twodash") +
  geom_path(color=colorblind_pal()(3)[2]) + 
  scale_linetype_manual(name="", labels=parse(text = paste0('A > ', amp_choices)), 
                        values = c("solid", "dashed")) + 
  xlab("Recall = DiffR found/DiffR present") + theme(legend.position = "none") +
...
df6_5 <- expand.grid(schwarz_wt_cutoff = grid_points,
                  amp_cutoff = amp_choices)

roc <- vapply(seq(nrow(df6_5)),
              function(i) {
                if (df6_5[i, "amp_cutoff"] == amp_choices[1]) {
                    est_dr_genes <- filter(mod_sel_ex_2_1, weights > df6_5[i, "schwarz_wt_cutoff"] & category %in% c("loss", "gain", "change"))
                } else {
                    est_dr_genes <- filter(mod_sel_ex_2_2, weights > df6_5[i, "schwarz_wt_cutoff"] & category %in% c("loss", "gain", "change"))
                }
                tp <- length(intersect(est_dr_genes$symbol, rhythmic_in_A))
                fn <- length(setdiff(rhythmic_in_A, est_dr_genes$symbol))
                fp <- length(setdiff(est_dr_genes$symbol, rhythmic_in_A))
                return(c(TP = tp, FP = fp, FN = fn))
              }, FUN.VALUE = numeric(3))
df6_5 <- cbind(df6_5, t(roc))
...
df6_5ext <- df6_5 %>% group_by(amp_cutoff) %>% 
  arrange(desc(TP)) %>% 
  filter(row_number()==1) %>%
...
colnames(df6_5ext) <- gsub("extn.", "", colnames(df6_5ext))
df6_5 <- bind_rows(df6_5, data.frame(df6_5ext)) %>%
         group_by(amp_cutoff) %>% arrange(desc(TP))

f6_5 <- ggplot(df6_5, aes_(x=~TP/(TP+FN), y=~TP/(FP+TP), linetype=~factor(amp_cutoff))) + 
  geom_hline(aes_(yintercept=~N/(N + nrow(pvm_ex_2))), color="grey60", linetype="twodash") +
  geom_path(color=colorblind_pal()(3)[3]) + scale_linetype_manual(name="", labels=parse(text = paste0('A > ', amp_choices)), 
                        values = c("solid", "dashed")) + 
 coord_cartesian(xlim=c(0,1.0), ylim=c(0,1.0)) + theme(legend.position = "none") +
  xlab("") + ylab("")

df6_6 <- expand.grid(schwarz_wt_cutoff = grid_points,
                  amp_cutoff = amp_choices)

roc <- vapply(seq(nrow(df6_6)),
              function(i) {
                if (df6_6[i, "amp_cutoff"] == amp_choices[1]) {
                    est_dr_genes <- filter(mod_sel_ex_2_1, weights > df6_6[i, "schwarz_wt_cutoff"] & category %in% c("loss", "gain", "change"))
                } else {
                    est_dr_genes <- filter(mod_sel_ex_2_2, weights > df6_6[i, "schwarz_wt_cutoff"] & category %in% c("loss", "gain", "change"))
                }
                tp <- length(intersect(est_dr_genes$symbol, rhythmic_in_A_large_amp))
                fn <- length(setdiff(rhythmic_in_A_large_amp, est_dr_genes$symbol))
                fp <- length(setdiff(est_dr_genes$symbol, rhythmic_in_A_large_amp))
                return(c(TP = tp, FP = fp, FN = fn))
              }, FUN.VALUE = numeric(3))
df6_6 <- cbind(df6_6, t(roc))

df6_6ext <- df6_6 %>% group_by(amp_cutoff) %>% 
  arrange(desc(TP)) %>% 
  filter(row_number()==1) %>%
...
colnames(df6_6ext) <- gsub("extn.", "", colnames(df6_6ext))
df6_6 <- bind_rows(df6_6, data.frame(df6_6ext)) %>%
         group_by(amp_cutoff) %>% arrange(desc(TP))

f6_6 <- ggplot(df6_6, aes_(x=~TP/(TP+FN), y=~TP/(FP+TP), linetype=~factor(amp_cutoff))) + 
  geom_hline(aes_(yintercept=~N/(N + nrow(pvm_ex_2))), color="grey60", linetype="twodash") +
  geom_path(color=colorblind_pal()(3)[3]) + scale_linetype_manual(name="", labels=parse(text = paste0('A > ', amp_choices)), 
                        values = c("solid", "dashed")) + 
 coord_cartesian(xlim=c(0,1.0), ylim=c(0,1.0))

f6_1 + f6_3 + f6_5 + f6_2 + f6_4 + f6_6 + plot_layout(nrow = 2, guides='collect') + plot_annotation(tag_levels = "A")
...
f2_2 <- ggplot(df_2_2) +
  ...
  scale_shape_identity() + ylab("DiffR transcripts")

```

```{r odd-even-correct-plots,  eval=FALSE}
...
```{r FigureS2, fig.height=4, fig.width=16/3, eval=FALSE}
...
f2_4 <- ggplot(tidyr::gather(df_2_4, type, value, -amp_cutoff)) +
  ...
  scale_fill_manual(name = "", values = c("white", "grey70")) +
   scale_x_discrete(labels = parse(text = paste0('A >', amp_choices)), name = NULL) + 
  theme(legend.key.height = unit(0.2, "lines")) + ylab("DiffR transcripts")
...
# print(length(union(setdiff(results_emahan_dodr$symbol, results_emahan$symbol), setdiff(results_emahan$symbol, results_emahan_dodr$symbol)))/length(union(results_emahan_dodr$symbol, results_emahan$symbol))) # due to higher power of RAIN
...
f3_1 <- f3_1 + geom_linerange(aes(y=len, color=method), xmin=0, xmax=0.1, 
                   data = df3_1, size=0.5) +
  geom_text(aes(y=len, label=method), x=0.06, data=df3_1, hjust=0, vjust=-0.3, size=9*0.35)
...
f3_2 <- ggplot(df3_2, aes(ymin=ymin, ymax=ymax, xmin=2, xmax=3, fill=category)) + 
...
data_eckel <- y_nutr_1[hfd_genes_ensembl$symbol,] %>% 
...
data_quag <- y_nutr_2_abund[hfd_genes_ensembl$symbol,] %>% 
...
df3_5 <- bind_rows(data_eckel, data_quag)

label_one <- function(labels) return(list(labels$symbol))
          
f3_5 <- ggplot(df3_5, aes(time, value, linetype=grp, color=type)) + 
  geom_point(size = 0.3) +
  geom_smooth(formula = 'y~x', method = "loess", size=0.4, se=FALSE, span=0.75) +
  facet_wrap(~ symbol + type, scales = "free_y", ncol = 6, labeller = label_one) + 
...
```{r Figure3, fig.height=8, fig.width=6.5}
...
f3_1 + f3_2 + f3_5 + gtable_cbind(f3_4_1, f3_4_2) + f3_3 + plot_layout(design = layout, widths = c(0.8,1), heights = c(0.66,1,0.66)) + plot_annotation(tag_levels = "A")
...
```{r keto_figs}
df4_1 <- bind_rows(data.frame(category = table(results_keto$category), method = "limma"),
...
```{r Figure4, fig.height=4, fig.width=6.5}
...
f4_3_1 <- tableGrob(df4_3_1, theme = ttheme_default(base_size = 8), cols = c("KEGG", "qvalue"), rows = NULL)
f4_3_2 <- tableGrob(df4_3_2, theme = ttheme_default(base_size = 8), cols = c("Hallmark (MSigDB)", "qvalue"), rows = NULL)
...
df4_4 <- y_keto[ifng_genes_ensembl$symbol,] %>% 
...
f4_4 <- ggplot(df4_4, aes(time, value, linetype=grp)) + 
  geom_point(size = 0.3) +
...
f4_1 + f4_2 + gtable_rbind(f4_3_1, f4_3_2) + f4_4 + plot_layout(design = layout, heights = c(1,0.3), widths = c(0.9,0.8,1)) + plot_annotation(tag_levels = "A")
...
dfS4_4 <- y_keto_iec[keto_iec_ensembl$symbol,] %>% 
...
df5_1 <- bind_rows(data.frame(category = table(results_pei$category), method = "voom"),
...
```{r Figure5, fig.height=4.5, fig.width=6.5}
...
df5_3_1 <- head(go_pei, n=3) %>%
           mutate(Description = str_wrap(Description, width=20),
                  qvalue = scales::scientific(qvalue, digits = 3)) %>%
           dplyr::select(Description, qvalue)
df5_3_2 <- head(hallmark_pei, n=10) %>%
           filter(qvalue<0.05) %>%
           mutate(Description = gsub("_", " ", sub("HALLMARK","",Description)),
                  Description = str_wrap(str_to_sentence(Description), width=20),
                  qvalue = scales::scientific(qvalue, digits = 3)) %>%
...
f5_3_1 <- tableGrob(df5_3_1, theme = ttheme_default(base_size = 8), cols = c("GO (BP)", "qvalue"), rows = NULL)
f5_3_2 <- tableGrob(df5_3_2, theme = ttheme_default(base_size = 8), cols = c("Hallmark genes \n (MSigDB)", "qvalue"), rows = NULL)
...
df5_4 <- y_pei_abund[kras_genes_ensembl$symbol,] %>% 
...
f5_4 <- ggplot(df5_4, aes(time, value, linetype=grp)) + 
  geom_point(size = 0.3) +
  geom_smooth(formula = 'y~x', method = "loess", size=0.4, se=FALSE, span=0.75, color="black") +
  facet_wrap(~ symbol, scales = "free_y", ncol = 6, labeller = label_one) + 
...
f5_1 + f5_2 + gtable_rbind(f5_3_1, f5_3_2) + f5_4 + plot_layout(design = layout, widths = c(1,0.9,1), heights = c(1,1)) + plot_annotation(tag_levels = "A")
```
